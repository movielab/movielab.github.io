...
<div class="bd">
            <div class="TpRwCont cont">
                <main>
                    ...
                    <section>
                        <div class="Top">
                            <h1><?php echo $row['name'];?> <?php echo __('menu','popular_tv') ;?></h1>
                        </div>
                        <div>
                            <p> <?php echo __('utilities','movie_recommendation') ;?> <strong> <?php echo $row['name'];?> </strong> <?php echo __('utilities','des_2') ;?>o</p> 
                        </div>
                        <ul class="MovieList Rows AX A06 B04 C03 E20">
                                <?php $network = $singlen['id'];$movie = deqila_data_network('network',$network,$page); if( is_array($movie['result']) ) { foreach ( $movie['result'] as $item ) : if($item['vote_count'] == 0) { $item['new_vote_count'] = 1;}else{ $item['new_vote_count'] = $item['vote_count'];} if($item['vote_count'] == 0) { $item['new_vote_average'] = 1;}else{ $item['new_vote_average'] = $item['vote_average'];} ?>
