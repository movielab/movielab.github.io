<?php /** * @package deqila.id */ if ( empty( $_GET['page'] ) ) { $page = 1; $hala = ''; }else{ $page = $_GET['page']; $hala = ' - '. $_GET['page'];} $title = ucwords ( strtolower ( str_replace('-',' ',$_GET['term'] ) ) ); $hack_title = __('details','keyword') .' '. $title . $hala . ' - ' . $options['weblogo']; $titulo = __('details','keyword') .' '. $title . $hala; $hack_description = __('details','keyword') . $title . ' - ' . $options['weblogo'] . ' - ' . $options['webdescription']; $hack_keywords = strtolower( strtolower ( str_replace('-',' ',$_GET['term'] ) ) ). ','; include dirname(__DIR__) .'/header.php'; ?><meta itemprop="image" content="<?php echo get_webinfo('theme_url') ;?>img/og-image-default.jpg"/><meta property="og:image" content="<?php echo get_webinfo('theme_url') ;?>img/og-image-default.jpg"/><meta name="twitter:image" content="<?php echo get_webinfo('theme_url') ;?>img/og-image-default.jpg"/><?php include(ABSPATH . THMPATH . $options['theme_name'] . '/header2.php')?>
<div class="bd">
            <div class="TpRwCont cont">
                <main>
                    ...
                    <section>
                        <div class="Top">
                            <h1><?php echo __('menu','most_viewed') ;?> <?php echo $titulo;?></h1>
                        </div>
                        <div>
                            <p><?php echo __('utilities','movie_recommendation') ;?> <strong><?php echo $titulo;?> </strong> <?php echo __('utilities','des_2') ;?> </p>
                        </div>
                        <ul class="MovieList Rows AX A06 B04 C03 E20">
                               <?php if ( empty( $_GET['page'] ) ) { $page = 1; }else{ $page = $_GET['page'];}$movie = deqila_data_keyword($_GET['id'],$page,'GetKeywordMovies');if( is_array($movie['result']) ) {foreach ( $movie['result'] as $key => $item ) {if($item['vote_count'] == 0) { $item['new_vote_count'] = 1;}else{ $item['new_vote_count'] = $item['vote_count'];}if($item['vote_count'] == 0) { $item['new_vote_average'] = 1;}else{ $item['new_vote_average'] = $item['vote_average'];}?>
                                ...
                                <?php 
                                    ...
                                    ?>
                                <?php if ( empty( $_GET['page'] ) ) { $page = 1; }else{ $page = $_GET['page']; } $movie = deqila_data_keyword_tv($_GET['id'] ,$page,'GetKeywordTVShow'); if( is_array($movie['result']) ) { foreach ( $movie['result'] as $key => $item ) { if($item['vote_count'] == 0) { $item['new_vote_count'] = 1;}else{ $item['new_vote_count'] = $item['vote_count'];} if($item['vote_count'] == 0) { $item['new_vote_average'] = 1;}else{ $item['new_vote_average'] = $item['vote_average'];} ?>
                                <li class="xxx TPostMv">
                                    <div class="TPost C post type-post status-publish format-standard has-post-thumbnail hentry">
                        ...
                        <nav class="navigation pagination">
                            ...
                            $link = '/keyword/'.$_GET['term'].'/'.$_GET['o']; 
                            $pagination = new CSSPagination($totalResults, $limit, $link,'?' ); 
                            }else{ 
                            $link = '/'.$_GET['language'].'/keyword/'.$_GET['term'].'/'.$_GET['o']; 
                            $pagination = new CSSPagination($totalResults, $limit, $link,'?' );} 
