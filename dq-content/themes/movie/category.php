                ...
                <main>
                    ...
                    <section>
                        ...
                        <ul class="MovieList Rows AX A06 B04 C03 E20">
                               <?php 
                                    $movie = unserialize( deqila_data_genre('g_'.$_GET['o'],$_GET['o'] ,$page) ); 
                                    if( is_array($movie['result']) ) { 
                                        foreach ( $movie['result'] as $key => $item )  { 
                                            if($item['vote_count'] == 0) { 
                                            $item['new_vote_count'] = 1;  
                                            }else{ 
                                            $item['new_vote_count'] = $item['vote_count'];
                                            } if($item['vote_count'] == 0) { 
                                            $item['new_vote_average'] = 1;  
                                            }else{ 
                                            $item['new_vote_average'] = $item['vote_average'];
                                            } 
                                ?>
                        ...
                        <nav class="navigation pagination">
                            ...
                            $link = '/genres/'.$_GET['term'].'/'.$_GET['o']; 
                            $pagination = new CSSPagination($totalResults, $limit, $link,'?' ); 
                            }else{ 
                            $link = '/'.$_GET['language'].'/genres/'.$_GET['term'].'/'.$_GET['o']; 
                            $pagination = new CSSPagination($totalResults, $limit, $link,'?' );} 
