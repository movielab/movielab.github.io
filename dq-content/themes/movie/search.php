...
<meta itemprop="image" content="<?php echo get_webinfo('theme_url') ;?>img/og-image-default.png"/>
<meta property="og:image" content="<?php echo get_webinfo('theme_url') ;?>img/og-image-default.png"/>
<meta name="twitter:image" content="<?php echo get_webinfo('theme_url') ;?>img/og-image-default.png"/>
            ...
            <div class="TpRwCont cont">
                <main>
                    ...
                    <section>
                        ...
                        <div>
                            <p><?php echo __('utilities','movie_recommendation') ;?> <strong><?php echo ucwords(get_search_query());?> </strong> <?php echo __('utilities','des_2') ;?> </p>
                        </div>
                        <ul class="MovieList Rows AX A06 B04 C03 E20">
                               <?php if( is_array($output1['result']) ) { echo $movies['sorry']; foreach ( $output1['result'] as $key => $item )  { if($item['vote_count'] == 0) { $item['new_vote_count'] = 1;  }else{ $item['new_vote_count'] = $item['vote_count'];} if($item['vote_count'] == 0) { $item['new_vote_average'] = 1;  }else{ $item['new_vote_average'] = $item['vote_average'];} ?>
                               ...
                               <?php if ($key == 19)break; } } ?>
                                <?php } ?>
                                <?php if( is_array($output2['result']) ) {?><?php foreach ( $output2['result'] as $key => $item )  { if($item['vote_count'] == 0) { $item['new_vote_count'] = 1;  }else{ $item['new_vote_count'] = $item['vote_count'];} if($item['vote_count'] == 0) { $item['new_vote_average'] = 1;  }else{ $item['new_vote_average'] = $item['vote_average'];} ?>
                                <li class="xxx TPostMv">
                                    <div class="TPost C post type-post status-publish format-standard has-post-thumbnail hentry">
                        ...
                        <nav class="navigation pagination">
                            <?php 
                            if ($output1['total_results'] > 19) : require_once( ABSPATH. '/dq-includes/o_pagination.php'); 
                            if ($output1['total_results'] > 1000) : $totalResults = 1000; else: $totalResults =$output1['total_results']; endif; 
                            $limit= 20; 
                            if ( empty( $_GET['language'] ) ) { 
                            $link = '/'.$options['search'].'/'.get_search_query(); 
                            $pagination = new CSSPagination($totalResults, $limit, $link,'?' ); 
                            }else{ 
                            $link = '/'.$_GET['language'].'/'.$options['search'].'/'.get_search_query(); 
                            $pagination = new CSSPagination($totalResults, $limit, $link,'?' );} 
