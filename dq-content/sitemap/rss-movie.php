...
$movie = unserialize( deqila_data_movie('movie','1','getNowPlayingMovies') );if( is_array($movie['result']) ): {foreach ( $movie['result'] as $rss) {
    $date = date("r");
    $link = movie_single($rss['id'],$rss['title']); 
...
<?php 
$tanggal_cal = date( 'Y-m-d' );
$movie = unserialize( deqila_data_movie('movie','2','getNowPlayingMovies') );if( is_array($movie['result']) ): {foreach ( $movie['result'] as $rss) {
    $date = date("r");
    $link = movie_single($rss['id'],$rss['title']); 
    echo '<item><title>'.esc_attr($rss['title']).'</title><link>'.$link.'</link><description>' .esc_attr($rss['overview']).'</description><guid>'.$link.'</guid><pubDate>'.$date.'</pubDate></item>';
} }
